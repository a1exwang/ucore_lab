...
print_stackframe(void) {
	...
	uint32_t *current_args;
	int i, j;
	int argc = 4;

	current_ebp = read_ebp();
	current_eip = read_eip();
	current_args = (uint32_t*)(current_ebp + 8);

	for (i = 0; i < STACKFRAME_DEPTH; ++i) {
		cprintf("ebp: 0x%0x, eip: 0x%0x\nargs: ", current_ebp, current_eip);
		for (j = 0; j < argc; ++j) {
			cprintf("0x%0x ", current_args[j]);
		}
		cprintf("\n---------------------------------------\n");
		print_debuginfo(current_eip);
		current_eip = *((uint32_t*) (current_ebp + 4));
